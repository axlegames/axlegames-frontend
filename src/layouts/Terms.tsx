...
const Terms = (props: any) => {
  ...
  return (
    <Box fontWeight={"bold"}>
      ...
      <Text>
        do not purchase AXLE token if you are not an expert in dealing with
        cryptographic tokens and blockchain-based software systems. prior to
        purchasing AXLE, you should carefully consider the terms listed below
        ...
        you, you should not purchase AXLE.
      </Text>
      <Text>
        purchaser agrees to buy, and company agrees to sell, the AXLE tokens in
      ...
      <Box>
        {terms.map((term) => {
          return (
            <Box key={term.title}>
              <Text fontSize={"2xl"}>{term.title}</Text>
              <Text fontSize={"m"}>{term.content}</Text>
              <Text fontSize={"sm"}>{term.sub} </Text>
            </Box>
          );
        })}
      </Box>
      <Flex pt={8} pb={4} justifyContent="flex-end">
        <NeuButton
          shadow={theme.newSecondaryShadow}
          bg={theme.neuSecondaryBg}
